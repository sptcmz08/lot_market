...
@section('content')
    ...
    <div class="camera-modal" id="browser-camera" aria-hidden="true">
        <div class="camera-dialog" role="dialog" aria-modal="true" aria-label="ถ่ายรูป">
            ...
            <div class="camera-error" id="camera-error">เปิดกล้องไม่ได้ กรุณาอนุญาตการใช้กล้องแล้วลองใหม่</div>
...
document.addEventListener('DOMContentLoaded', () => {
    const removeSelectedFile=(form, input, index)=>{
        ...
        const dt=new DataTransfer();[...(input.files||[])].forEach((file,fileIndex)=>{if(fileIndex!==index)dt.items.add(file);});input.files=dt.files;update(form);
    };
    const update=form=>{
